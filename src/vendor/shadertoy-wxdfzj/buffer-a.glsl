...
vec4 ApplyToneMapping(vec4 Result,float shift) {
    float ColorSum    = Result.r + Result.b + Result.g + 1e-6;
    float RedFactor   = 3.0 * Result.r / ColorSum;
    float BlueFactor  = 3.0 * Result.b / ColorSum;
    float GreenFactor = 3.0 * Result.g / ColorSum;
...
void transformKerrSchild_YSpin(inout vec4 X, in float r_sign, inout vec4 P, float M, float a, float Q, bool out_to_in) {
    ...
    float safe_Delta = sign(Delta) * max(abs_Delta, 1e-16); if (safe_Delta == 0.0) safe_Delta = 1e-16;

    float r3 = r * r * r; float D = r3 * r + a2 * y * y; float safe_D = max(D, 1e-12);

    vec3 grad_r = vec3(r3 * x / safe_D, r * (r*r + a2) * y / safe_D, r3 * z / safe_D);

    float delta_disc = M2 - a2 - Q2; float F_r = 0.0; float g_r = 0.0; float abs_Delta_safe = max(abs_Delta, 1e-16);

    if (delta_disc > 1e-16) {
        float K = sqrt(delta_disc); float r_plus = M + K; float r_minus = M - K;
        float frac = abs(r - r_plus) / max(abs(r - r_minus), 1e-16);
        float log_frac = log(max(frac, 1e-16));
        F_r = 2.0 * M * log(abs_Delta_safe) + ((2.0 * M2 - Q2) / K) * log_frac;
        g_r = (a / K) * log_frac;
    ...
    float g_prime = 2.0 * a / safe_Delta - 2.0 * a / (r * r + a2);
    float Ly = z * px - x * pz; 
               ...
               float ThetaInShell, inout float RayMarchPhase, vec2 WP_CamX, vec2 WP_CamY, inout vec2 StokesQU) 
{
    ...
    while (TraveledDist < TotalDist && SafetyLoopCount < MaxLoops) {
        ...
        if (shouldSample) {
            ...
            float InnerCloudRadialTerm = 1.0 - 5.0 * pow((PosR - InterRadius) / min(OuterRadius - InterRadius, 12.0), 2.0);
            float InnerCloudBound = max(GeometricThin, Thin * 1.0) * max(0.0, InnerCloudRadialTerm);
            ...
            if (abs(PosY) < max(GeometricThin * 1.5, max(0.0, InnerCloudBound)) && PosR < OuterRadius && PosR > InterRadius) {
                 ...
                 if ((abs(PosY) < PerturbedThickness) || (abs(PosY) < InnerCloudBound)) {
                     ...
                     if (abs(k_cubed) < 0.001 * u * u * u) SpiralTheta = -16.9705627 / u * (1.0 - 0.25 * k_cubed * pow(1.0/u, 3.0) + 0.142857 * pow(k_cubed * pow(1.0/u, 3.0), 2.0));
                         ...
                         float k_root = sign(k_cubed) * pow(abs(k_cubed), 0.33333333);
                         float k_root_sq = pow(k_root, 2.0);
                         float u_plus_k_root_sq = pow(u + k_root, 2.0);
                         SpiralTheta = (5.6568542 / k_root) * (0.5 * log(max(1e-9, (PosR - k_root*u + k_root_sq) / max(1e-9, u_plus_k_root_sq))) + 1.7320508 * (atan(2.0*u - k_root, 1.7320508 * k_root) - 1.5707963));
                     }
                     ...
                     float LocalDiskTemperature = pow(DiskTemperatureArgument * pow(1.0 / max(1e-6, PosR), 3.0) * max(1.0 - sqrt(InterRadius / max(1e-6, PosR)), 0.000001), 0.25);
                     float VisionTemperature = LocalDiskTemperature * pow(FreqRatio, RedShiftColorExponent); 
                     float BrightWithoutRedshift = (0.05 * min(OuterRadius / 1000.0, 1000.0 / OuterRadius) + 0.55 / exp(5.0 * EffectiveRadius) * mix(0.2 + 0.8 * abs(local_Dir.y), 1.0, clamp(GeometricThin - 0.8, 0.2, 1.0))) * pow(LocalDiskTemperature / PeakTemperature, BlackbodyIntensityExponent); 
                     
                     float Density = DenAndThiFactor; vec4 SampleColor = vec4(0.0);

                     if (abs(PosY) < PerturbedThickness) {
                         float OuterRadiusPow07 = pow(OuterRadius, 0.7);
                         ...
                         SampleColor = vec4(GenerateAccretionDiskNoise(vec3(0.1 * (PosR + 0.25 / 3.0 * EmissionTime), 0.1 * PosY, 0.02 * OuterRadiusPow07 * PosTheta), NoiseLevel + 2.0 - Levelmut, NoiseLevel + 4.0 - Levelmut, 80.0 - Conmut)); 
                         if(PosTheta + kPi < 0.1 * kPi) { SampleColor *= (PosTheta + kPi) / (0.1 * kPi); SampleColor += (1.0 - ((PosTheta + kPi) / (0.1 * kPi))) * vec4(GenerateAccretionDiskNoise(vec3(0.1 * (PosR + 0.25 / 3.0 * EmissionTime), 0.1 * PosY, 0.02 * OuterRadiusPow07 * (PosTheta + 2.0 * kPi)), NoiseLevel + 2.0 - Levelmut, NoiseLevel + 4.0 - Levelmut, 80.0 - Conmut)); }
                         if(PosR > max(0.15379 * OuterRadius, 0.15379 * 64.0)) {
                             float TimeShiftedRadiusTerm = PosR * (4.65114e-6) - 0.1 / 3.0 * EmissionTime;
                             float Spir = (GenerateAccretionDiskNoise(vec3(0.1 * (TimeShiftedRadiusTerm - 0.08 * OuterRadius * PosLogarithmicTheta), 0.1 * PosY, 0.02 * OuterRadiusPow07 * PosLogarithmicTheta), NoiseLevel + 2.0 - Levelmut, NoiseLevel + 3.0 - Levelmut, 80.0 - Conmut)); 
                             if(PosLogarithmicTheta + kPi < 0.1 * kPi) { Spir *= (PosLogarithmicTheta + kPi) / (0.1 * kPi); Spir += (1.0 - ((PosLogarithmicTheta + kPi) / (0.1 * kPi))) * (GenerateAccretionDiskNoise(vec3(0.1 * (TimeShiftedRadiusTerm - 0.08 * OuterRadius * (PosLogarithmicTheta + 2.0 * kPi)), 0.1 * PosY, 0.02 * OuterRadiusPow07 * (PosLogarithmicTheta + 2.0 * kPi)), NoiseLevel + 2.0 - Levelmut, NoiseLevel + 3.0 - Levelmut, 80.0 - Conmut)); }
                         ...
                         float RelativeDiskHeight = clamp(abs(PosY) / PerturbedThickness, 0.0, 1.0);
                         SampleColor.xyz *= Density * 1.4 * max(0.0, (0.2 + 2.0 * sqrt(max(0.0, RelativeDiskHeight * RelativeDiskHeight + 0.001))));
                     ...
                     if (abs(PosY) < InnerCloudBound) {
                         float DustIntensity = max(1.0 - pow(PosY / (GeometricThin * max(InnerCloudRadialTerm, 0.0001)), 2.0), 0.0);
                         if (DustIntensity > 0.0) {
                             vec2 InnerCloudReferenceDir = vec2(cos(0.666666 * InnerCloudTimePhase), sin(0.666666 * InnerCloudTimePhase));
                             float InnerCloudTheta = Vec2ToTheta(SamplePos.zx, InnerCloudReferenceDir);
                             vec3 InnerCloudNoisePos = vec3(1.5 * fract((1.5 * InnerCloudTheta + InnerCloudTimePhase) / 2.0 / kPi) * 2.0 * kPi, PosR, PosY);
                             float InnerCloudNoise = GenerateAccretionDiskNoise(InnerCloudNoisePos, 0.0, 6.0, 80.0);
                             SampleColor += 0.02 * vec4(vec3(DustIntensity * InnerCloudNoise), 0.2 * DustIntensity * InnerCloudNoise * sqrt(max(0.0, 1.0001 - local_Dir.y * local_Dir.y)));
                         }
                     }
                     ...
                     float RemainingAlpha = 1.0 - CurrentResult.a;
                     float AttenuationR = pow(RemainingAlpha, aR);
                     float AttenuationG = pow(RemainingAlpha, aG);
                     float AttenuationB = pow(RemainingAlpha, aB);
                     float Denominator = StepColor.r * AttenuationR + StepColor.g * AttenuationG + StepColor.b * AttenuationB;
                     if (Denominator > 0.000001) {
                         float Sum_rgb = (StepColor.r + StepColor.g + StepColor.b) * AttenuationG;
                         float ContributionR = Sum_rgb * StepColor.r * AttenuationR / Denominator;
                         float ContributionG = Sum_rgb * StepColor.g * AttenuationG / Denominator;
                         float ContributionB = Sum_rgb * StepColor.b * AttenuationB / Denominator;
                         float ContributionSum = ContributionR + ContributionG + ContributionB;
                         CurrentResult.r += ContributionR * pow(3.0 * ContributionR / ContributionSum, Saturation);
                         CurrentResult.g += ContributionG * pow(3.0 * ContributionG / ContributionSum, Saturation);
                         CurrentResult.b += ContributionB * pow(3.0 * ContributionB / ContributionSum, Saturation);
                     }
                     CurrentResult.a += StepColor.a * RemainingAlpha;
                 }
            }
        }
    }
...
TraceResult TraceRay(vec2 FragUv, vec2 Resolution, mat4 iInverseCamRot, vec4 iBlackHoleRelativePosRs, vec4 iBlackHoleRelativeDiskNormal, vec4 iBlackHoleRelativeDiskTangen, float iUniverseSign)
{
    ...
    if (iEnableHeatHaze == 1 && iInWhichUniverse==0 && CurrentUniverseSign>0.0) {
        ...
        // The disk haze mask is exactly zero at and beyond this radial cutoff.
        // A normalized probe ray can reduce its xz radius by at most totalProbeDist.
        // Jets also contribute exactly zero below their existing accretion threshold.
        bool canReachDiskHaze = length(pos_Rg_Start.xz) <= iOuterRadiusRs * HAZE_RADIAL_EXPAND + totalProbeDist;
        bool canProduceJetHaze = iAccretionRate >= 1e-2;

        if ((canReachDiskHaze || canProduceJetHaze) && IsInHazeBoundingVolume(pos_Rg_Start, totalProbeDist, iOuterRadiusRs)) {
    ...
    while (bShouldContinueMarchRay) {
        ...
        vec4 P_contra = RaiseIndex(P_cov, geo);
        {
            ...
            if (current_Sum > 2.0 * dot(abs(test_P_contra), vec4(1.0))) {
                X = test_X; P_cov = test_P; isoutgoing = !isoutgoing; geo = test_geo;
                P_contra = test_P_contra;
        ...
        if(iGrid==0 && iWhitehole==0) {
            ...
            if (CurrentUniverseSign > 0.0 && !bIsNakedSingularity) {
                if (geo.r < min(iInterRadiusRs, min(min(CameraStartR - 0.001, TerminationR + 0.2), ProgradePhotonRadius - 0.001))) {
                    if (CurrentDr > 1e-4) { bShouldContinueMarchRay = false; bWaitCalBack = false; break; }
                }
            }
        }
        ...
        if (P_contra.w > 10000.0*iQuality && !bIsNakedSingularity && CurrentUniverseSign > 0.0) { bShouldContinueMarchRay = false; bWaitCalBack = false; break; }
...
void mainImage( out vec4 FragColor, in vec2 FragCoord )
{
    ...
    mat3 InvCamRotMat = transpose(CamRotMat);
    mat4 iInverseCamRot = mat4(CamRotMat); 

    vec3 RelPos = InvCamRotMat * (-CamPosWorld);
    vec4 iBlackHoleRelativePosRs = vec4(RelPos, 0.0);
    
    vec3 DiskNormalWorld = vec3(0.0, 1.0, 0.0);
    vec3 DiskTangentWorld = vec3(1.0, 0.0, 0.0);
    vec4 iBlackHoleRelativeDiskNormal = vec4(InvCamRotMat * DiskNormalWorld, 0.0);
    vec4 iBlackHoleRelativeDiskTangen = vec4(InvCamRotMat * DiskTangentWorld, 0.0);
